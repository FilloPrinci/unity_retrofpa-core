...
- **Audio**: `AudioManager` + `AudioProfile` (global UI hover/confirm sounds,
  ...
  - `UIButtonSound`: add to a `Button` to play the profile's
    hover sound (mouse pointer-enter, or gamepad/keyboard navigation selection;
    not repeated when a click selects it) and confirm
    (`onClick`) sound.
